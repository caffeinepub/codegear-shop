...
export class Backend implements backendInterface {
    ...
    async addHobby(arg0: string): Promise<bigint> {
    ...
    async getHobbies(): Promise<Array<Hobby>> {
        if (this.processError) {
            try {
                const result = await this.actor.getHobbies();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
    ...
    async removeHobby(arg0: bigint): Promise<boolean> {
        if (this.processError) {
            try {
